...
static char *
readstdin(Bool blank) {
	static char *maxname = NULL;
	char *p, buf[1024];
	unsigned int len = 0, max = 0, block = 0;
	Item *i, *new;
	i = 0;
	
	if(blank) return maxname;

	while(fgets(buf, sizeof buf, stdin)) {
		len = strlen(buf);
		if (buf[len - 1] == '\n')
			buf[len - 1] = 0;
	   if(len <= 1) continue;//if item is "", don't add it
		p = estrdup(buf);
		if(max < len) {
			maxname = p;
			max = len;
...
main(int argc, char *argv[]) {
	Bool bottom = False;
	Bool blank = False;//are we *not* going to recieve input?
	char *font = FONT;
	char *maxname;
	...
	for(i = 1; i < argc; i++)
		if(!strcmp(argv[i], "-b")) {
			...
			eprint("dmenu-"VERSION", © 2006-2007 Anselm R. Garbe, Sander van Dijk - singpolyma\n");
		else
			usage();
	setlocale(LC_CTYPE, "");
	dpy = XOpenDisplay(0);
