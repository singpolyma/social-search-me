...
QStringList SxWResult::row( int row ) {
	...
	for(int i = 0; i < str.size(); i++) {
		...
		if(i+1 < str.size() && in_field && start_quote && str.at(i) == '\"' && str.at(i+1) == ',') {//if we are in a field that started with a quote and the next two characters are ",
			in_field = false;
			start_quote = false;
			rtrn.append(tmp);
			tmp = "";
	...
	if(start_quote && str.at(str.size()-1) != '\"') {//\n in side a field
		contents.replace(row, contents.at(row) + contents.at(row+1));
		contents.removeAt(row+1);
		return this->row(row);
	}//end if \n inside a field
	if(start_quote) tmp.chop(1);
	rtrn.append(tmp);
	return rtrn;
}
