	...
	*/
	...
	class hKit
	{
		...
		public function getByString($profile='', $input_xml='')
		{
			...
			$s1	= $this->processNodes($doc[0], $this->classes);
			$s1	= $this->postProcess($profile, $s1);
			$s		= $this->processNodes($doc[1], $this->classes);
			$s		= $this->postProcess($profile, $s);
			
			return array('preferred' => $s1, 'all' => $s);
		...
		private function loadDoc($input_xml, $fragment=false)
		{
			...
			if($xml)
				return 	array(
$xml->xpath("//$this->preferred_root[contains(concat(' ',normalize-space(@class),' '),' $this->root_class ')]"),
$xml->xpath("//*[contains(concat(' ',normalize-space(@class),' '),' $this->root_class ')]")
			);
			else
				return array();
