	...
	class server {
		...
		function __construct($name) {
			...
			if(!is_numeric($name)) {
				...
				$data = mysql_query("SELECT server_id,server_name,previous_day,day_length,previous_week,week_length,initial_gold,initial_city_count,city_names,initial_city_population,city_cost FROM servers WHERE server_id=".mysql_real_escape_string($name,$db)." LIMIT 1",$db) or die(mysql_error());
