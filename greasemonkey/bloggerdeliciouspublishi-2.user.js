// ==UserScript==
// @name          Blogger del.icio.us publishing and post categorizing helper (with trackback) for the new Blogger
...
function parseDetailsCallback(data) {
   ...
   var myid = getBlogId();
   var url = data.entry.link[0].href;
   ...
   for(var i in data.entry.category) {
      tags.push(data.entry.category[i].term);
...
function moveTagsToHeader()
{
  ...
  var divs = getFields();
  divs.trackback.value = GM_getValue('trackbackSave' + divs.title.value, '');
...
function injectTagsField(divs)
{
  ...
  $('titles').getElementsByTagName('tbody')[0].appendChild( trackbackRow );
  if( blogthis ) {
