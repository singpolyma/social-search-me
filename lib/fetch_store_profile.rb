...
begin
	...
	warn 'No rubygems, if requires fail, this is probably why.'
end
require 'hpricot'
require 'uri'
require 'cgi'
require 'mysql'
require 'time'
require File.dirname(__FILE__) + '/representative_hcard'
require File.dirname(__FILE__) + '/get_doc'
require File.dirname(__FILE__) + '/hcard_urls'

# PRE: requires a global $queue array
def fetch_store_profile(the_url, db)
	...
	warn 'bad data'
			...
			unless verified.nil?
				...
				return nil ## This URL no longer belongs to anyone, die
			end
	end
	...
	# Add to end of queue for update
	$queue << the_url
